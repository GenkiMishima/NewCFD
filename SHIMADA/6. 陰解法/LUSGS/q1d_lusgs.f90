...
!
      ...
      dimension RHS(3,n),nu_max(n),factor(n), dq$(3,n), abar(n),dfdq(3,3,n)
	  dimension ds(3), a$(3,3), b$(3,3), c$(3,3)
!
...
! read input data
!
      open(99,file='input13.txt',status='unknown')
      read(99,input)
      read(99,flow)
	  read(99,space)
	  read(99,boundary)
...
!
		do i=1, n-1
			nu_max(i)=abs(fnu(i))+fna(i)
			abar(i)=0.5*(A(i)+A(i+1))
			factor(i)=Vol(i)/dt+abar(i)*nu_max(i)
...
!
			...
			a$(1,2) = 0.
			a$(1,3) = 0.
			a$(2,1) = dt*ds(1)/(1.-dt*ds(2))
			a$(2,2) = 1./(1.-dt*ds(2))
			a$(2,3) = dt*ds(3)/(1.-dt*ds(2))
			a$(3,1) = 0.
			a$(3,2) = 0.
			a$(3,3) = 1.
!
			dq$(1:3,i)=MATMUL(a$,rhs(1:3,i))
		end do
!
...
!
		do i=1, n-1
			v=fnu(i)
			H=0.5*v**2+fna(i)**2/gm
			dfdq(1,1,i)=0.
			dfdq(1,2,i)=1.
			dfdq(1,3,i)=0.
			dfdq(2,1,i)=0.5*(gam-3.)*v*v
			dfdq(2,2,i)=(3.-gam)*v
			dfdq(2,3,i)=gm
			dfdq(3,1,i)=0.5*v*(-2.*H+gm*v*v)
			dfdq(3,2,i)=H-gm*v*v
			dfdq(3,3,i)=gam*v
		end do
!
		do i=2, n-1
			b$ = 0.5*dfdq(1:3,1:3,i-1)
			b$(1,1)=b$(1,1)+0.5*nu_max(i-1)
			b$(2,2)=b$(2,2)+0.5*nu_max(i-1)
			b$(3,3)=b$(3,3)+0.5*nu_max(i-1)
!
			fact=abar(i-1)/factor(i)
			dq$(1:3,i)=dq$(1:3,i)+fact*MATMUL(b$,dq$(1:3,i-1))
		end do
!
! the 3rd step : backword substitution
!
		do i=n-2, 1, -1
			c$ = 0.5*dfdq(1:3,1:3,i+1)
			c$(1,1)=c$(1,1)-0.5*nu_max(i+1)
			c$(2,2)=c$(2,2)-0.5*nu_max(i+1)
			c$(3,3)=c$(3,3)-0.5*nu_max(i+1)
!
			fact=abar(i+1)/factor(i)
			dq$(1:3,i)=dq$(1:3,i)-fact*MATMUL(c$,dq$(1:3,i+1))
		end do
!
! Update field
!
...
! output
!
         write(51,'(i6,a1,e16.8)') ic,',',log10(residual+1.e-20)
