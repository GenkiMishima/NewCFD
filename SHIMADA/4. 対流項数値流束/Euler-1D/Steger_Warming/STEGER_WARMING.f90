...
!****************************************************************************
!
!  PROGRAM: STEGER_WARMING
!
! 1-D Euler Solver
! Steger-Warming's Scheme : Flux Vector Splliting Scheme for Euler Equations
!
...
!
         ...
         w_L(3,1)=pL
!
! Numerical Flux Evaluation  : Steger-Warming's Scheme
!
         do i=1, n
...
!
			...
			amm3=abs(1.+emR)
!
			f1p=0.25/gam*aL*w_L(1,i)*(2.*emL*gam+amp1+2.*gm*amp2+amp3)
			f1m=0.25/gam*aR*w_R(1,i)*(2.*emR*gam-amm1-2.*gm*amm2-amm3)
!
			f2p=0.25/gam*aL**2*w_L(1,i)*(2.+2.*emL**2*gam+(-1.+emL)*amp1 & 
			                                             +2.*emL*gm*amp2 &
														 +( 1.+emL)*amp3 )
			f2m=0.25/gam*aR**2*w_R(1,i)*(2.+2.*emR**2*gam-(-1.+emR)*amm1 &
			                                             -2.*emR*gm*amm2 &
														 -( 1.+emR)*amm3 )
!
			f3p=0.125/gam/gm*aL**3*w_L(1,i)*( 2.*emL*(2.+emL**2*gm)*gam &
							                +(2.+(-2.+emL)*emL*gm)*amp1 &
							                       +2.*(emL*gm)**2*amp2 &
							                +(2.+( 2.+emL)*emL*gm)*amp3)
			f3m=0.125/gam/gm*aR**3*w_R(1,i)*( 2.*emR*(2.+emR**2*gm)*gam &
							                -(2.+(-2.+emR)*emR*gm)*amm1 &
							                       -2.*(emR*gm)**2*amm2 &
							                -(2.+( 2.+emR)*emR*gm)*amm3)
!
