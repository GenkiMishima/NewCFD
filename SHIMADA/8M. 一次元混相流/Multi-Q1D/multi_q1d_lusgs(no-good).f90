...
!****************************************************************************
      ...
      real*8 kappa, nu_max_f, nu_max_p
      parameter(n=201)
!
      dimension x(n),q_f(3,-1:n+1), q_p(3,-1:n+1) ,f_f(3,n), f_p(3,n) 
	  dimension w_L(6,0:n+1),w_R(6,0:n+1)
	  dimension A(n), Vol(n)
	  dimension source_f(3), source_p(3)
      dimension dw_b(6), dw_f(6), w_c(6)
      dimension rhs_f(3),rhs_p(3), dq_f$(3,n), dq_p$(3,n), abar(n)
	  dimension dfdq_f(3,3,n), dfdq_p(3,3,n)
	  dimension nu_max_f(n), nu_max_p(n), factor_f(n), factor_p(n)
	  dimension ds(3), a_f$(3,3), b_f$(3,3), c_f$(3,3)
...
!
      fnp(i)=gm*(q_f(3,i)-0.5*q_f(2,i)**2/q_f(1,i))
      fnu(i)=q_f(2,i)/q_f(1,i)
      fna(i)=sqrt(gam*fnp(i)/q_f(1,i))
	  fnt(i)=(q_f(3,i)/q_f(1,i)-0.5*fnu(i)**2)/cv
...
!
	  ...
	  tp0=tpL
	  a0=sqrt(gam*gasc*tL)
	  ae0=a0*sqrt((gam+aloadratio*akappa)/gam/(1.+aloadratio*akappa)/(1.+aloadratio))
...
!
	  ...
	  ip=ip+1
	  write(ip,'(6e16.8)') (q_f(1,i)/r0,fnu(i)/ae0,fnt(i)/t0,q_p(1,i)/rp0,fnup(i)/ae0,fntp(i)/tp0,i=1,n-1)
...
!			r_out=2.*q_f(1,n-1)-q_f(1,n-2)
!			u_out=2.*fnu(n-1)-fnu(n-2)
			r_out=1.5*q_f(1,n-1)-0.5*q_f(1,n-2)
		...
		q_f(1,0)=r_in
		q_f(2,0)=r_in*u_in
		q_f(3,0)=p_in/gm+0.5*r_in*u_in**2
		q_p(1,0)=rp_in
		q_p(2,0)=rp_in*up_in
		...
		q_f(1,n)=r_out
		q_f(2,n)=r_out*u_out
		q_f(3,n)=p_out/gm+0.5*r_out*u_out**2
		q_p(1,n)=rp_out
...
! Time step evaluation
         ...
         do i=1,n-1
			...
			up0=0.5*(fnup(i-1)+abs(fnup(i-1)))
			up1m=0.5*(fnup(i)-abs(fnup(i)))
			up1p=0.5*(fnup(i)+abs(fnup(i)))
			up2=0.5*(fnup(i+1)-abs(fnup(i+1)))
			dtpr=q_p(1,i)*Vol(i)/max((-q_p(1,i-1)*up0+q_p(1,i)*abs(up1m))*a(i) &
			                         +(q_p(1,i)*up1p-q_p(1,i+1)*abs(up2))*a(i+1),0.)
			dtpe=q_p(3,i)*Vol(i)/max((-q_p(3,i-1)*up0+q_p(3,i)*abs(up1m))*a(i) &
			                         +(q_p(3,i)*up1p-q_p(3,i+1)*abs(up2))*a(i+1),0.)
!			dtpr=Vol(i)/(abs(up1m)*a(i)+up1p*a(i+1))
!			dtpe=dtpr
		
			dti=min(dtf,dtpr,dtpe)
            dtmin=min(dtmin,dti)
         end do
         dt=cfl*dtmin
         time=time+dt
...
!
         do i=1, n-1
            ...
            dw_f(1) = q_f(1,i+1) - q_f(1,i  )
            dw_f(2) = fnu(i+1) - fnu(i  )
            dw_f(3) = fnp(i+1) - fnp(i  )
			dw_f(4) = q_p(1,i+1) - q_p(1,i  )
...
!
            ...
            f_f(1,i)=0.5*(f1L+f1R-d1) * A(i)
            f_f(2,i)=0.5*(f2L+f2R-d2) * A(i)
            f_f(3,i)=0.5*(f3L+f3R-d3) * A(i)
!!
...
!
         do i=1, n-1
			source_f(1:3)=0.
			source_p(1:3)=0.
			source_f(2)=dt*fnp(i)*(A(i+1)-A(i))/Vol(i)
!
		    u =fnu(i)
			up=fnup(i)
		    u_rel=u-up
			reynolds=q_f(1,i)*abs(u_rel)*dp/amyu
			
			sos=sqrt(gam*fnp(i)/q_f(1,i))
			amach=abs(u_rel)/sos
			...
			if(reynolds.gt.0.) then
			   ...
			   cfactor=alambda/(q_f(1,i)*dp*sos*cp)
			   aNusselt0=2.
			end if
			...
			temp_p=fntp(i)
			stemp=temp_g-temp_p
			if(abs(stemp).le.1.e-12) stemp=0.
			source_f(3)=-q_p(1,i)*cc*stemp*dtbytau_t
...
!			   tau_v=min(tau_v_eq,tau_v)
			   dtbytau_v=dt/tau_v

			   source_f(2)=source_f(2)-q_p(1,i)*u_rel*dtbytau_v
			   source_f(3)=source_f(3)-q_p(1,i)*u_rel*up*dtbytau_v
		    ...
		    source_p(3)=-source_f(3)

!
! Right-Hand Side of Eq. (6-9)
!
			rhs_f(1:3)=-f_f(1:3,i+1)+f_f(1:3,i)
			rhs_p(1:3)=-f_p(1:3,i+1)+f_p(1:3,i)
			rhs_f(1:3)=rhs_f(1:3)+Vol(i)*source_f(1:3)/dt
...
!
			akai=q_p(1,i)/q_f(1,i)
			umax_p=abs(fnup(i))
			umax_f=abs(fnu(i))+fna(i)
			distance=umax_p*max(tau_v,tau_t)
!			write(6,*) distance, dx
			if(distance.lt.dx) then
				nu_max_f(i)=max(umax_p,umax_f)
				nu_max_p(i)=nu_max_f(i)
			else
				nu_max_f(i)=umax_f
				nu_max_p(i)=umax_p
			end if

			beta_f=1.+dt/dx*nu_max_f(i)
			beta_p=1.+dt/dx*nu_max_p(i)
!			beta_f=1.
!			beta_p=1.

			abar(i)=0.5*(A(i)+A(i+1))
			factor_f(i)=Vol(i)/dt+abar(i)*nu_max_f(i)
			factor_p(i)=Vol(i)/dt+abar(i)*nu_max_p(i)
			rhs_f(1:3)=rhs_f(1:3)/factor_f(i)
...
!
			...
			a_p$=0.
			a_f$(1,1) = 1.
			a_f$(2,1) = dta*ds(1)/(1.-dta*ds(2))
			a_f$(2,2) = 1./(1.-dta*ds(2))
			a_f$(2,3) = dta*ds(3)/(1.-dta*ds(2))
			a_f$(3,3) = 1.
			a_p$(1,1) = 1.
			a_p$(2,2) = 1.
			a_p$(3,3) = 1.
!
			dq_f$(1:3,i)=MATMUL(a_f$,rhs_f(1:3))
			rhs_f(1:3)=dq_f$(1:3,i)
			dq_p$(1:3,i)=rhs_p(1:3)

			temp_g=fnt(i)
			up=fnup(i)

			fac=1./ ( dtbytau_v*(beta_f+beta_p*akai)+ beta_f*beta_p )

			dq_f$(2,i)= fac * (rhs_f(1) * dtbytau_v * u * beta_p * akai &
			                  +rhs_f(2) * (dtbytau_v * beta_f + beta_f * beta_p) &
							  -rhs_p(1) * dtbytau_v * u * beta_p &
							  +rhs_p(2) * dtbytau_v * beta_p )
			dq_p$(2,i)= fac *(-rhs_f(1) * dtbytau_v * u * beta_f * akai &
			                  +rhs_f(2) * dtbytau_v * beta_f * akai &
							  +rhs_p(1) * dtbytau_v * u * beta_f &
							  +rhs_p(2) * (dtbytau_v * beta_p * akai + beta_f * beta_p) )

			dsdq31=-((cv*temp_g-u**2/2.)*akai*akappa*dtbytau_t + u*up*akai*dtbytau_v) / beta_f
			dsdq32=-(u*akappa*akai*dtbytau_t - up*akai*dtbytau_v) / beta_f
			dsdq33=1. + akappa*akai*dtbytau_t / beta_f
			dsdq34=-((up**2/2. - cv*temp_g*akappa)*dtbytau_t - up**2*dtbytau_v) / beta_f
			dsdq35=-(-up*dtbytau_t + (-u + 2.*up)*dtbytau_v) / beta_f
			dsdq36=-dtbytau_t / beta_f

			dsdq61= ((cv*temp_g-u**2/2.)*akai*akappa*dtbytau_t + u*up*akai*dtbytau_v) / beta_p
			dsdq62= (u*akappa*akai*dtbytau_t - up*akai*dtbytau_v) / beta_p
			dsdq63= -akappa*akai*dtbytau_t / beta_p
			dsdq64= ((up**2/2. - cv*temp_g*akappa)*dtbytau_t - up**2*dtbytau_v) / beta_p
			dsdq65= (-up*dtbytau_t + (-u + 2.*up)*dtbytau_v) / beta_p
			dsdq66=1.+dtbytau_t / beta_p

			rhs_f(3)=rhs_f(3)-(dsdq31*dq_f$(1,i)+dsdq32*dq_f$(2,i)+dsdq34*dq_p$(1,i)+dsdq35*dq_p$(2,i))
			rhs_p(3)=rhs_p(3)-(dsdq61*dq_f$(1,i)+dsdq62*dq_f$(2,i)+dsdq64*dq_p$(1,i)+dsdq65*dq_p$(2,i))

			dq_f$(3,i)= (dsdq66*rhs_f(3) - dsdq36*rhs_p(3))/ (-dsdq36*dsdq63 + dsdq33*dsdq66)
			dq_p$(3,i)= (dsdq63*rhs_f(3) - dsdq33*rhs_p(3))/ ( dsdq36*dsdq63 - dsdq33*dsdq66)
		end do
...
!
		do i=1, n-1
			...
			dfdq_f(1,2,i)=1.
			dfdq_f(1,3,i)=0.
			dfdq_f(2,1,i)=0.5*(gam-3.)*u*u
			dfdq_f(2,2,i)=(3.-gam)*u
			dfdq_f(2,3,i)=gm
			dfdq_f(3,1,i)=0.5*u*(-2.*H+gm*u*u)
			dfdq_f(3,2,i)=H-gm*u*u
			dfdq_f(3,3,i)=gam*u
...
!
		do i=2, n-1
			b_f$ = 0.5*dfdq_f(1:3,1:3,i-1)
			b_f$(1,1)=b_f$(1,1)+0.5*nu_max_f(i-1)
			b_f$(2,2)=b_f$(2,2)+0.5*nu_max_f(i-1)
			b_f$(3,3)=b_f$(3,3)+0.5*nu_max_f(i-1)
...
!
			fact_f=abar(i-1)/factor_f(i)
			fact_p=abar(i-1)/factor_p(i)
			dq_f$(1:3,i)=dq_f$(1:3,i)+fact_f*MATMUL(b_f$,dq_f$(1:3,i-1))
...
!
		do i=n-2, 1, -1
			c_f$ = 0.5*dfdq_f(1:3,1:3,i+1)
			c_f$(1,1)=c_f$(1,1)-0.5*nu_max_f(i+1)
			c_f$(2,2)=c_f$(2,2)-0.5*nu_max_f(i+1)
			c_f$(3,3)=c_f$(3,3)-0.5*nu_max_f(i+1)
...
!
			fact_f=abar(i+1)/factor_f(i)
			fact_p=abar(i+1)/factor_p(i)
			dq_f$(1:3,i)=dq_f$(1:3,i)-fact_f*MATMUL(c_f$,dq_f$(1:3,i+1))
...
!
		 ...
		 fact=1.
         do i=1, n-1
			alpha=abs(q_p(1,i)*cc*fntp(i)) / (abs((0.5*fnup(i)**2-cc*fntp(i))*dq_p$(1,i)) &
				+abs(fnup(i)*dq_p$(2,i))+abs(dq_p$(3,i)))
			beta=abs(q_p(1,i))/ (abs(dq_p$(1,i))+1.e-30)
			fact=min(fact,alpha*0.8,beta*0.8)
		 end do
		...
		tpmin=fntp(0)
         do i=1, n-1
			eng_old=q_f(3,i)
			q_f(1:3,i)=q_f(1:3,i)+dq_f$(1:3,i) * fact
			...
			residual=residual+(q_f(3,i)-eng_old)**2
         end do
