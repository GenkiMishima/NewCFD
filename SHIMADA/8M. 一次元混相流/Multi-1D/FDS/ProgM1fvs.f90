...
!
      ...
      namelist /input/ CFL, tend, toutput, phi, kappa, delta, imp
...
!
            ...
            f(4,i)=rpL$*upL$ + rpR$*upR$
            f(5,i)=rpL$*upL$**2 + rpR$*upR$**2
            f(6,i)=rpL$*upL$*epL$ + rpR$*upR$*epR$
...
!
		 do i=1, n-1
			...
			temp_p=fntp(i)
			stemp=temp_g-temp_p
!			if(abs(stemp).lt.1.e-12) then
!			write(6,*) i,stemp
!			stemp=0.
!			end if
			source(3,i)=-q(4,i)*cc*stemp*dtbytau_t
			...
			if(u_rel.ne.0.) then
			   ...
			   source(2,i)=-q(4,i)*u_rel*dtbytau_v
			   source(3,i)=source(3,i)-q(4,i)*u_rel*up*dtbytau_v
			...
			akai=q(4,i)/q(1,i)
			rhs(1:6)=-dt/dx*(f(1:6,i+1)-f(1:6,i))+source(1:6,i)
			dq(1:6)=rhs(1:6)

			if(imp.eq.1) then
			dq(2)=(rhs(2)*(dtbytau_v+1.)+dtbytau_v*(rhs(5)-rhs(4)*u+rhs(1)*u*akai)) / &
			      (dtbytau_v+1.+dtbytau_v*akai)
			dq(5)=(rhs(4)*u*dtbytau_v+rhs(5)+(rhs(2)+rhs(5)-rhs(1)*u)*dtbytau_v*akai) / &
			      (dtbytau_v+1.+dtbytau_v*akai)
			dsdq31=-(((cv*temp_g - u**2/2.)*akai*akappa)*dtbytau_t + (u*up*akai)*dtbytau_v)
			dsdq32=-((u*akappa*akai)*dtbytau_t - (up*akai)*dtbytau_v)
			dsdq33=1.-(-(akappa*akai)*dtbytau_t)
			dsdq34=-(-((-up**2/2. + cv*temp_g*akappa)*dtbytau_t) - up**2*dtbytau_v)
			dsdq35=-(-(up*dtbytau_t) + (-u + 2*up)*dtbytau_v)
			dsdq36=-dtbytau_t

			dsdq61= (((cv*temp_g - u**2/2.)*akai*akappa)*dtbytau_t + (u*up*akai)*dtbytau_v)
			dsdq62= ((u*akappa*akai)*dtbytau_t - (up*akai)*dtbytau_v)
			dsdq63= (-(akappa*akai)*dtbytau_t)
			dsdq64= (-((-up**2/2. + cv*temp_g*akappa)*dtbytau_t) - up**2*dtbytau_v)
			...
			do k=1,6
				if(abs(dq(k)).lt.1.e-10) dq(k)=0.
			end do
			q(1:6,i)=q(1:6,i)+dq(1:6)
!			write(6,*) source(1:6,1)
		end do
...
!
         if(time.ge.tout) then
            ...
            ip=ip+1
            write(6,*) 'Time=',time
	        write(ip,'(6e16.8)') (q(1,i)/rL,fnu(i)/sqrt(gam*pL/rL),fnt(i)/tL, &
			    q(4,i)/rpL,fnup(i)/sqrt(gam*pL/rL),fntp(i)/tpL,i=1,n-1)
         end if
      end do
