...
!
            ...
            upH=fctL*w_L(5,i) + fctR*w_R(5,i)

            dq4=w_R(4,i)-w_L(4,i)
            dq5=w_R(4,i)*w_R(5,i)-w_L(4,i)*w_L(5,i)
            dq6=w_R(4,i)*(cc*w_R(6,i)+0.5*w_R(5,i)**2) &
			   -w_L(4,i)*(cc*w_L(6,i)+0.5*w_L(5,i)**2)

! Entropy Fix Considered
            auph=abs(uph)
			if(auph.lt.delta_p) auph=(uph**2+delta_p**2)/(2.*delta_p)

            f4L=w_L(4,i)*w_L(5,i)
            f5L=w_L(4,i)*w_L(5,i)**2
            f6L=w_L(4,i)*w_L(5,i)*(cc*w_L(6,i)+0.5*w_L(5,i)**2)

            f4R=w_R(4,i)*w_R(5,i)
            f5R=w_R(4,i)*w_R(5,i)**2
            f6R=w_R(4,i)*w_R(5,i)*(cc*w_R(6,i)+0.5*w_R(5,i)**2)

            f(4,i)=0.5*(f4L+f4R-auph*dq4)
            f(5,i)=0.5*(f5L+f5R-auph*dq5)
            f(6,i)=0.5*(f6L+f6R-auph*dq6)

			up_p=0.5*(w_L(5,i)+abs(w_L(5,i)))
			up_m=0.5*(w_R(5,i)-abs(w_R(5,i)))
			f(4,i)=up_p*w_L(4,i)+up_m*w_R(4,i)
			f(5,i)=up_p*w_L(4,i)*w_L(5,i)+up_m*w_R(4,i)*w_R(5,i)
			f(6,i)=up_p*w_L(4,i)*(cc*w_L(6,i)+0.5*w_L(5,i)**2) &
			      +up_m*w_R(4,i)*(cc*w_R(6,i)+0.5*w_R(5,i)**2)
         end do
!
! Source Term
!
...
!			q(1:6,i)=q(1:6,i)+sign(1.,dq(1:6))*max(1.e-30,abs(dq(1:6)))
