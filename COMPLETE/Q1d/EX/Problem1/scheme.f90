...
use prmtr
implicit none
   double precision, intent(in) , dimension(1) ::nv
   double precision, intent(in) , dimension(3) ::wl,wr
   double precision, intent(out), dimension(3) ::flux


   double precision ul,vl,rhol,gmmal,unl,phil(3),al,Ml,betal,pl
   double precision ur,vr,rhor,gmmar,unr,phir(3),ar,Mr,betar,pr
   ...
   phil(1)=1.d0
   phil(2)=ul
   temp0=pl/(gamma-1d0)+0.5d0*rhol*(ul**2)
   phil(3)=(temp0+pl)/rhol !enthalpy
   ...
   phir(1)=1
   phir(2)=ur
   temp0=pr/(gamma-1d0)+0.5d0*rhor*(ur**2)
   phir(3)=(temp0+pr)/rhor!enthalpy
   ...
   !set dm
   m_tilde=min(1d0,1d0/a_bar*sqrt(0.5d0*(ul**2++ur**2)))
   chi=(1d0-m_tilde)**2
...
!
subroutine SLAU_FLUX(w,Flux)
use prmtr
implicit none
integer i,j
double precision,dimension(3,0:CellNum) :: w,Flux,w_left,w_right
...
!   !X_MUSCL{{{
   ...
   !$omp parallel do private(i)
   do i=1, CellNum-1
      call MUSCLlimiter(w(:,i-1), w(:,i), w(:,i+1), w_left(:,i), w_right(:,i))
   enddo
   !$omp end parallel do
   !MUSCL}}}
   do i=1, CellNum
      call SLAU(w_right(:,i-1),w_left(:,i),Flux(:,i),1d0)
   end do
!
!!   !Y_MUSCL{{{
!   w_down(:,:,:)=w(:,:,:)
